(function(){
  ...
  function enhanceSelect(select){
    ...
    trigger.addEventListener("click",function(event){event.stopPropagation();var opening=menu.hidden;customSelects.forEach(closeCustomSelect);if(opening){menu.hidden=false;wrap.classList.add("open");var row=wrap.closest(".form-row");if(row)row.classList.add("dropdown-active");trigger.setAttribute("aria-expanded","true");var first=menu.querySelector(".selected")||menu.querySelector(".custom-select-option");if(first)first.focus()}});
    wrap.addEventListener("keydown",function(event){var items=[].slice.call(menu.querySelectorAll(".custom-select-option")),current=items.indexOf(document.activeElement);if(event.key==="ArrowDown"){event.preventDefault();items[Math.min(current+1,items.length-1)].focus()}if(event.key==="ArrowUp"){event.preventDefault();items[Math.max(current-1,0)].focus()}if(event.key==="Escape"){closeCustomSelect(wrap);trigger.focus()}});
